...
class Rewards:
    ...
    def _reward_arm_vel_control(self):
        if self.env.plan_actions.shape[1] < 4:
            return torch.zeros(self.env.num_envs, device=self.env.device)
    
        linv_vel_error = torch.abs(
            self.env.plan_actions[:, 2] - self.env.base_lin_vel[:, 0] * self.env.obs_scales.lin_vel
        )
        ang_vel_error = torch.abs(
            self.env.plan_actions[:, 3] - self.env.base_ang_vel[:, 2] * self.env.obs_scales.ang_vel
        )
    
        return linv_vel_error + ang_vel_error
    
    def _reward_arm_orientation_control(self):
        if self.env.plan_actions.shape[1] < 2:
            return torch.zeros(self.env.num_envs, device=self.env.device)
    ...
    def _reward_arm_control_limits(self):
        ...
        out_of_limits = -(self.env.plan_actions[:, 0] - self.env.cfg.commands.limit_body_pitch[0]).clip(max=0.)
        out_of_limits += (self.env.plan_actions[:, 0] - self.env.cfg.commands.limit_body_pitch[1]).clip(min=0.)
        out_of_limits += -(self.env.plan_actions[:, 1] - self.env.cfg.commands.limit_body_roll[0]).clip(max=0.)
    ...
    def _reward_arm_action_smoothness_1(self):
        arm_start = self.env.num_actions_loco
        arm_end = arm_start + self.env.num_actions_arm
    
        diff = torch.square(
            self.env.joint_pos_target[:, arm_start:arm_end]
            - self.env.last_joint_pos_target[:, arm_start:arm_end]
        )
        diff = diff * (self.env.last_actions[:, arm_start:arm_end] != 0)  # ignore first step
        return torch.sum(diff, dim=1)
    ...
    def _reward_arm_action_smoothness_2(self):
        ...
        diff = torch.square(
            self.env.joint_pos_target[:, arm_start:arm_end]
            - 2 * self.env.last_joint_pos_target[:, arm_start:arm_end]
            + self.env.last_last_joint_pos_target[:, arm_start:arm_end]
        )
        diff = diff * (self.env.last_actions[:, arm_start:arm_end] != 0)  # ignore first step
        diff = diff * (self.env.last_last_actions[:, arm_start:arm_end] != 0)  # ignore second step
        return torch.sum(diff, dim=1)
