"""
gen_grasp_lift_v9.py
====================
Final conservative grasp demo:
- fixed robot spawn (no random x/y offset)
- scripted reach and gripper close
- box starts near verified grasp point
- very small, conservative lift
"""
...
VIDEO_OUT = "/root/RoboDuet/b2z1_grasp_lift_v9.mp4"
FPS = 30
BOX_HALF = 0.025
...
# Measured in box-env at stable reach+close.
BOX_START = torch.tensor([-0.085, 0.022, 0.829], device='cuda:0')
...
def capture(env, cam, cp):
    ...
    env.gym.set_camera_location(
        cam,
        env.envs[0],
        gymapi.Vec3(rx + 1.45, ry + 0.85, rz + 0.58),
        gymapi.Vec3(rx - 0.05, ry, rz + 0.36),
    )
...
def main():
    print("=" * 60)
    print("  B2Z1 Grasp Lift v9  (conservative grasp + tiny lift)")
