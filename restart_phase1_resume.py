...
RUN_DIR = "/root/RoboDuet/runs/b2z1_grasp_real_phase1/dummy-l3rzdo8v_seed6118"
...
cd /root/RoboDuet
python auto_train_grasp_real.py --headless --no_wandb --num_learning_iterations 300 --num_envs 256 --run_name b2z1_grasp_real_phase1_resume --resume --arm_resume_path {ARM_CKPT} --dog_resume_path {DOG_CKPT} > grasp_real_phase1_resume.log 2>&1
echo \"EXIT_CODE=$?\" >> grasp_real_phase1_resume.log
"""

    sftp = ssh.open_sftp()
    with sftp.file("/root/RoboDuet/run_grasp_real_phase1_resume.sh", "w") as f:
        f.write(launcher)
    sftp.close()

    ssh.exec_command("pkill -f auto_train_grasp_real.py || true")
    ssh.exec_command("rm -f /root/RoboDuet/grasp_real_phase1_resume.log")
    time.sleep(1)
    ssh.exec_command("nohup bash /root/RoboDuet/run_grasp_real_phase1_resume.sh &")
    time.sleep(3)
