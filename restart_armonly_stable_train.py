...
cd /root/RoboDuet
python auto_train_grasp_armonly.py --headless --no_wandb --num_learning_iterations 300 --num_envs 256 --run_name b2z1_grasp_armonly_stable > grasp_armonly_stable.log 2>&1
echo \"EXIT_CODE=$?\" >> grasp_armonly_stable.log
"""

    sftp = ssh.open_sftp()
    with sftp.file("/root/RoboDuet/run_grasp_armonly_stable.sh", "w") as f:
        f.write(launcher)
    sftp.close()

    ssh.exec_command("pkill -f auto_train_grasp_armonly.py || true")
    ssh.exec_command("rm -f /root/RoboDuet/grasp_armonly_stable.log")
    time.sleep(1)
    ssh.exec_command("nohup bash /root/RoboDuet/run_grasp_armonly_stable.sh &")
    time.sleep(3)
