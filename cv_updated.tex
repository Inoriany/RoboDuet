...
\begin{itemize}
  \item 实现 LoRA 微调、指令微调（Instruction Tuning）与 RLHF/DPO 对齐；将 Qwen2.5-0.5B 应用于 SFT 翻译任务，覆盖数据准备到评估的完整流程。
  \item 实验对比 Prompt Engineering、Chain-of-Thought、Self-Consistency 对推理性能的提升；分析 hallucination 成因与缓解策略。
\end{itemize}

\vspace{5pt}
\entry{基于强化学习的文本到图像生成优化}{01/2026 -- 06/2026}
\begin{itemize}
  \item 独立搭建端到端 RL 微调流水线，将 GRPO（组相对策略优化）应用于自回归视觉生成模型 Janus-Pro-1B，采用 LoRA 高效微调；设计 CLIP + 32B VLM（Qwen2.5-VL API）复合奖励模型，对物体、数量、属性绑定与空间关系进行细粒度语义评分。
  \item 实现两阶段训练循环（无梯度采样 + 梯度重评分）与 Classifier-Free Guidance；解决 fp16 溢出、显存不足等工程问题，实现单卡 T4/L4 可训练。
  \item 在 4 个基准上评测（TIFA、GenAI-Bench、T2I-CompBench、GenEval-2），\textbf{TIFA +3.4、GenAI-Bench +6.2}，定性分析显示计数错误减半。代码开源：\href{https://github.com/Inoriany/T2I-RL-Project}{github.com/Inoriany/T2I-RL-Project}。
\end{itemize}
