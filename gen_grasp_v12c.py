...
gen_grasp_v12c.py
=================
Static-object grasp-only demo.

Sequence:
- hidden preview finds a stable low grasp location
- object is placed statically on a pedestal
- arm moves toward the static object
- gripper closes
- robot holds the grasp pose without lifting
"""
...
VIDEO_OUT = "/root/RoboDuet/b2z1_grasp_v12c.mp4"
FPS = 30
...
PREVIEW_SAMPLE_STEPS = 36
RETURN_HIGH_STEPS = 120
MAX_PREVIEW_TRIES = 10
PREVIEW_MIN_BASE_H = 0.43
PREVIEW_MIN_GM_Z = 0.55
PREVIEW_MAX_GM_Z = 0.82
...
NUM_STEPS = HOLD0_STEPS + REACH_STEPS + CLOSE_STEPS + HOLD1_STEPS
...
def sample_preview(env, arm_policy, num_plan, steps):
    gm_samples = []
    h_samples = []
...
def run_preview_attempt(env, arm_policy, num_plan, idx):
    env.reset()
    env.commands_dog[:, :] = 0.0
    ...
    with torch.no_grad():
        ...
        gm_low, h_low = sample_preview(env, arm_policy, num_plan, PREVIEW_SAMPLE_STEPS)

    ok = (
        h_low >= PREVIEW_MIN_BASE_H and
        PREVIEW_MIN_GM_Z <= gm_low[2] <= PREVIEW_MAX_GM_Z
    )
    print(
        f"  Preview try {idx}: h={h_low:.3f} "
        f"gm=({gm_low[0]:.3f},{gm_low[1]:.3f},{gm_low[2]:.3f}) ok={ok}"
    )
    ...
    with torch.no_grad():
        for i in range(RETURN_HIGH_STEPS):
            ...
            step_env(env, arm_policy, num_plan, GRIPPER_OPEN, lerp(CMD_P_LOW, CMD_P_HIGH, frac))

    return {"gm_low": gm_low, "h_low": h_low}


def main():
    print("=" * 72)
    print("  B2Z1 Grasp v12c  (static object, no lift)")
    print("  Object remains static on pedestal until grasp hold")
    print("=" * 72)
    ...
    print("\n[3/4] Hidden preview to determine static grasp pose ...")
    preview = None
    for idx in range(1, MAX_PREVIEW_TRIES + 1):
        preview = run_preview_attempt(env, arm_policy, num_plan, idx)
    ...
    h_low = preview["h_low"]
    box_pos = [gm_low[0], gm_low[1], gm_low[2] + ATTACH_OFFSET_Z]
    pedestal_pos = [
        box_pos[0],
        box_pos[1],
        max(PEDESTAL_HALF_Z, box_pos[2] - BOX_HALF - PEDESTAL_HALF_Z),
    ]
    print(f"  chosen preview base_h={h_low:.3f}")
    print(f"  static box pose: ({box_pos[0]:.3f}, {box_pos[1]:.3f}, {box_pos[2]:.3f})")
    ...
    with torch.no_grad():
        for step in range(NUM_STEPS):
            ...
            step_env(env, arm_policy, num_plan, grip, cmd_p)
            ...
            if step == 0 or (step + 1) % 60 == 0 or step == NUM_STEPS - 1:
                ...
                target = [gm[0], gm[1], gm[2] + ATTACH_OFFSET_Z]
                d = dist3(box_pos, target)
                fps_now = (step + 1) / max(time.time() - t0, 1e-6)
                print(
                    f"  Step {step + 1:>4d}/{NUM_STEPS}  base_h={h:.3f}  "
                    f"box=({box_pos[0]:.3f},{box_pos[2]:.3f})  "
                    f"gm=({target[0]:.3f},{target[2]:.3f})  "
                    f"dist={d:.3f}  cmd_p={cmd_p:.3f}  grip={grip:.2f}  "
