"""
Launch training v8 — key change from v7:
  - FIX EE-frame reward gaming bug:
    xy_align, z_align, and close_bonus pinch now use WORLD-FRAME distances
    instead of EE-local-frame. This prevents the arm from learning to rotate
    the end-effector to cheat alignment rewards without actually approaching
    the object.
  - All other params same as v7:
    LR=1e-5, entropy=0.03, episode=2.4s, stage1_spawn_x=[0.32,0.36]
    fix_base_link=True, 256 envs, 5000 iters
    ppo.py: advantage norm + return normalization
    Start from original pretrained weights
"""
import paramiko
import time
import os
...
if ps.strip():
    ...
    print("  v7 killed successfully")

# ── 2. Backup v7 log ────────────────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 2: Backup v7 log")
run(f'cp {REMOTE_DIR}/grasp_armonly_v7.log {REMOTE_DIR}/grasp_armonly_v7.log.bak', 'backup')
print("  v7 log backed up")

# ── 3. Verify ppo.py has return normalization (from v6b) ─────────────────────
print("\n" + "=" * 60)
print("STEP 3: Verify ppo.py has return normalization")
...
if count >= 2:
    print("  ppo.py already has return normalization. OK.")
else:
...
# ── 4. Verify arm_ac.py is clean (no tanh bound) ────────────────────────────
print("\n" + "=" * 60)
print("STEP 4: Verify arm_ac.py has no tanh bound")
ac_path = f'{REMOTE_DIR}/go1_gym_learn/ppo_cse_automatic/arm_ac.py'
out = run(f'grep -n "tanh" {ac_path}', 'arm_ac tanh check')
if out.strip():
    print(f"  tanh references found (likely normal activation, not value bound):")
    for line in out.strip().split('\n'):
        print(f"    {line}")
else:
    print("  arm_ac.py is clean. OK.")

# ── 5. Upload v8 files (with world-frame reward fix) ────────────────────────
print("\n" + "=" * 60)
print("STEP 5: Upload v8 files")
print("  KEY CHANGE: xy_align, z_align, close_bonus pinch now use WORLD-FRAME")
files_to_upload = [
...
# Verify the fix is present on remote
print("\n  Verifying world-frame fix on remote...")
out = run(f'grep -n "WORLD-FRAME" {REMOTE_DIR}/real_grasp_rewards.py', 'verify fix')
if 'WORLD-FRAME' in out:
    print("  World-frame fix confirmed on remote!")
else:
    print("  ERROR: World-frame fix NOT found on remote!")

# ── 6. Launch v8 training ────────────────────────────────────────────────────
print("\n" + "=" * 60)
print("STEP 6: Launch v8 training")
print("  Params (same as v7 except reward fix):")
print("    LR: 1e-5")
print("    Entropy: 0.03")
print("    Object distance: ~0.34m (stage1)")
print("    Episode: 2.4s")
print("    Reward fix: xy_align/z_align/pinch in WORLD frame")
launch_cmd = (
    ...
    '--run_name b2z1_grasp_armonly_v8 '
    ...
    '--no_wandb > grasp_armonly_v8.log 2>&1 &'
)
...
ps = run('ps aux | grep auto_train | grep -v grep', 'process check')
if ps.strip():
    print("  v8 training launched!")
else:
    print("  WARNING: process not found! Checking log...")
    run('tail -20 /root/RoboDuet/grasp_armonly_v8.log', 'error log')

time.sleep(20)
out = run('tail -30 /root/RoboDuet/grasp_armonly_v8.log', 'initial log')

# Check first diagnostic
time.sleep(10)
out = run('grep "DIAG" /root/RoboDuet/grasp_armonly_v8.log', 'diagnostics')

sftp.close()
ssh.close()
print("\n" + "=" * 60)
print("DONE. Monitor with:")
print("  python check_v8.py")
