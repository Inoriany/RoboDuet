...
stdin, stdout, stderr = ssh.exec_command('wc -l /root/RoboDuet/grasp_armonly_v7.log 2>/dev/null; echo "---"; tail -30 /root/RoboDuet/grasp_armonly_v7.log 2>/dev/null')
log_out = stdout.read().decode()
print("\n=== Log tail ===")
print(log_out)

# Get diagnostics
stdin, stdout, stderr = ssh.exec_command('grep "DIAG" /root/RoboDuet/grasp_armonly_v7.log 2>/dev/null')
...
cmd = 'grep -E "close_bonus|Value function loss|Learning iteration|rew_total|rew_grasp_obj_dist|rew_grasp_xy_align|rew_grasp_z_align" /root/RoboDuet/grasp_armonly_v7.log'
