...
def train_go1(args):
    ...
    # ================================================================
    # CRITICAL FIX: override PD gains to physically correct values
    # asset_config.py hardcodes stiffness_leg=35, damping_leg=1 which
    # is too weak for the B2 robot (body collapses immediately).
    # legged_robot.py line 1345 reads dog.control.stiffness_leg,
    # NOT control.stiffness -- both must be set.
    # ================================================================
    KP_LEG = 200.0
    KD_LEG = 20.0
    ...
    global_switch.pretrained_to_hybrid_start = 2000 if args.resume else 10000
    ...
    if not args.debug:
        ...
        shutil.copyfile(
            f"{MINI_GYM_ROOT_DIR}/scripts/auto_train_kp200.py",
            f"{args.log_dir}/scripts/auto_train_kp200.py",
        )
        shutil.copyfile(
        ...
        shutil.copyfile(
            f"{MINI_GYM_ROOT_DIR}/go1_gym/envs/go1/asset_config.py",
            f"{args.log_dir}/scripts/asset_config.py",
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="B2Z1 Training Kp=200 Kd=20")
    parser.add_argument("--headless", action="store_true", default=False)
    parser.add_argument("--sim_device", type=str, default="cuda:0")
    parser.add_argument("--num_learning_iterations", type=int, default=50000)
