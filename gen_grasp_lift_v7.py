"""
gen_grasp_lift_v7.py
====================
Scripted grasp demo for presentation use.

This version does NOT use the oscillatory arm policy.
Instead it:
1. Keeps the quadruped stable with zero dog actions.
2. Drives the 6 arm joints along a scripted trajectory.
3. Closes the gripper with a separate DOF target.
4. Attaches the box to gripperMover after the close phase.
5. Lifts with a second scripted arm pose.

This is an animation-style demo intended to clearly show:
reach -> close -> lift.
"""
...
VIDEO_OUT = "/root/RoboDuet/b2z1_grasp_lift_v7.mp4"
FPS = 30
BOX_HALF = 0.025
...
ARM_LIFT  = torch.tensor([0.0, -3.2, 3.2, 0.0, 0.0, 0.0], device='cuda:0')
...
def set_gripper_target(env, value):
...
def step_env(env, arm_action, gripper_value):
    ...
    set_gripper_target(env, gripper_value)
...
def capture(env, cam, cp):
    ...
    env.gym.set_camera_location(
        ...
        gymapi.Vec3(rx + 0.15, ry, rz + 0.35),
    )
...
def main():
    print("=" * 60)
    print("  B2Z1 Grasp Lift v7  (scripted reach + close + attach + lift)")
    print(f"  steps: warmup={WARMUP_STEPS} total_recorded={NUM_STEPS}")
    print("=" * 60)
    ...
    reach_probe = ARM_REACH.unsqueeze(0)
    with torch.no_grad():
        for _ in range(100):
            step_env(env, ARM_REACH, GRIPPER_OPEN)

    box_pos = get_gripper_mover_pos(env)
    box_pos[2] -= BOX_HALF * 0.35
    move_box(env, box_pos)
    print(f"[OK] Box placed near gripper at ({box_pos[0]:.3f}, {box_pos[1]:.3f}, {box_pos[2]:.3f})")

    print("[3/3] Recording scripted grasp ...")
    frames = []
    t0 = time.time()
    attached = False
    last_box = box_pos.clone()
    ...
    with torch.no_grad():
        for step in range(NUM_STEPS):
            if step < HOLD0_STEPS:
                ...
                phase = "CLOSE"
            elif step < HOLD0_STEPS + REACH_STEPS + CLOSE_STEPS + HOLD1_STEPS:
                arm = ARM_REACH
                grip = GRIPPER_CLOSED
                phase = "HOLD1"
            elif step < HOLD0_STEPS + REACH_STEPS + CLOSE_STEPS + HOLD1_STEPS + LIFT_STEPS:
                frac = (step - HOLD0_STEPS - REACH_STEPS - CLOSE_STEPS - HOLD1_STEPS + 1) / LIFT_STEPS
            ...
            if (not attached) and phase in ("HOLD1", "LIFT", "HOLD2"):
                attached = True

            if attached:
                gm = get_gripper_mover_pos(env)
                new_box = gm.clone()
                new_box[2] -= BOX_HALF * 0.4
                move_box(env, new_box)
                last_box = new_box
            ...
            if step == 0 or (step + 1) % 60 == 0:
                ...
                print(
                    ...
                    f"box=({last_box[0]:.3f},{last_box[1]:.3f},{last_box[2]:.3f})  "
