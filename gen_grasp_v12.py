...
gen_grasp_v12.py
================
Static-on-pedestal grasp demo.

Goal:
- keep the object completely static before grasp
- make the arm move down toward that static object
- only let the object move after the gripper has closed

Approach:
- use the RL arm policy for the reach/lift motion
- first do a hidden preview reach in the SAME run
- sample the gripperMover position at the low pose after that preview
- place a small cube on a fixed pedestal at that sampled location
- record: high hold -> reach -> close -> hold -> lift
"""
...
VIDEO_OUT = "/root/RoboDuet/b2z1_grasp_v12.mp4"
FPS = 30
...
PREVIEW_SAMPLE_STEPS = 40
RETURN_HIGH_STEPS = 120
...
def capture(env, cam, cp, focus_pos):
    ...
    env.gym.set_camera_location(
        ...
        gymapi.Vec3(focus_pos[0], focus_pos[1], focus_pos[2] + 0.03),
    )
...
def main():
    print("=" * 72)
    print("  B2Z1 Grasp v12  (static pedestal before grasp)")
    print(f"  Box: {BOX_HALF * 2 * 100:.0f} cm cube")
    print(f"  Arm command: P {CMD_P_HIGH:.2f} -> {CMD_P_LOW:.2f} -> {CMD_P_HIGH:.2f}")
    print("  Object stays static on pedestal until after close")
    print("=" * 72)
    ...
    print("\n[3/4] Preview reach to determine static box location ...")
    env.reset()
    env.commands_dog[:, :] = 0.0
    ...
    with torch.no_grad():
        ...
        ee_samples = []
        h_samples = []
        for _ in range(PREVIEW_SAMPLE_STEPS):
            step_env(env, arm_policy, num_plan, GRIPPER_OPEN, CMD_P_LOW)
            gm_samples.append(get_gripper_mover_pos(env))
            ee_samples.append(get_ee_pos(env))
            h_samples.append(env.root_states[0, 2].item())

        n = len(gm_samples)
        gm_avg = [sum(v[i] for v in gm_samples) / n for i in range(3)]
        ee_avg = [sum(v[i] for v in ee_samples) / n for i in range(3)]
        base_h = sum(h_samples) / n

        box_target = [gm_avg[0], gm_avg[1], gm_avg[2] + ATTACH_OFFSET_Z]
        pedestal_pos = [
            box_target[0],
            box_target[1],
            max(PEDESTAL_HALF_Z, box_target[2] - BOX_HALF - PEDESTAL_HALF_Z),
        ]

        print(f"  preview base_h={base_h:.3f}")
        print(f"  gripperMover avg: ({gm_avg[0]:.3f}, {gm_avg[1]:.3f}, {gm_avg[2]:.3f})")
        print(f"  EE avg:           ({ee_avg[0]:.3f}, {ee_avg[1]:.3f}, {ee_avg[2]:.3f})")
        print(f"  box target:       ({box_target[0]:.3f}, {box_target[1]:.3f}, {box_target[2]:.3f})")
        print(f"  pedestal center:  ({pedestal_pos[0]:.3f}, {pedestal_pos[1]:.3f}, {pedestal_pos[2]:.3f})")
    ...
    with torch.no_grad():
        for step in range(NUM_STEPS):
            ...
            if phase in ("HOLD0", "REACH", "CLOSE", "HOLD1"):
                box_pos = list(box_target)
                smooth_pos = list(box_target)
            elif phase == "LIFT" and (step - T4) < ATTACH_BLEND_STEPS:
                frac = (step - T4 + 1) / ATTACH_BLEND_STEPS
                box_pos = lerp_vec(box_target, follow_target, frac)
                smooth_pos = list(box_pos)
            else:
                smooth_pos = [
                    (1 - FOLLOW_ALPHA) * smooth_pos[i] + FOLLOW_ALPHA * follow_target[i]
                    for i in range(3)
                ]
                box_pos = list(smooth_pos)

            set_actor_pose(env, 1, box_pos)
            set_actor_pose(env, 2, pedestal_pos)

            focus_pos = [
                0.60 * box_pos[0] + 0.40 * gm[0],
                0.60 * box_pos[1] + 0.40 * gm[1],
                0.60 * box_pos[2] + 0.40 * gm[2],
            ]
